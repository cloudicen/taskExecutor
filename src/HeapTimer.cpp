...
void HeapTimer::maintainHeap() {
    std::make_heap(this->timerHeap.begin(),this->timerHeap.end(),[](const TimerNode* a, const TimerNode* b){return (*a) > (*b);});
}

void HeapTimer::popHeap() {
    std::pop_heap(this->timerHeap.begin(),this->timerHeap.end(),[](const TimerNode* a, const TimerNode* b){return (*a) > (*b);});
...
int HeapTimer::getNextTickInterval() {
    ...
    if (!this->timerHeap.empty()){
        interval = std::chrono::duration_cast<std::chrono::milliseconds>(this->timerHeap.front()->expireTime - std::chrono::system_clock::now()).count();
        interval = interval < 0 ? 0 : interval;
    }
    return interval;
}

int HeapTimer::getTaskCount() {
